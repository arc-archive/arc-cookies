...
import { SessionCookieEventTypes, SessionCookieEvents, ExportEvents, TelemetryEvents } from '@advanced-rest-client/arc-events';
...
import { filterItems, compareCookies, computeHasTwoLines, applyListStyles } from './Utils.js';

/** @typedef {import('@advanced-rest-client/arc-events').SessionCookieDeletedEvent} SessionCookieDeletedEvent */
/** @typedef {import('@advanced-rest-client/arc-events').SessionCookieUpdatedEvent} SessionCookieUpdatedEvent */
...
/** @typedef {import('@advanced-rest-client/bottom-sheet').BottomSheetElement} BottomSheetElement */
 ...
 */
export class CookieManagerElement extends LitElement {
  static get styles() {
  ...
  [cookieActionsTemplate]() {
    const { compatibility, hasItems, hasSelection } = this;
    if (!hasItems) {
      return '';
    }
    return html`
  ...
  [searchBarTemplate]() {
    const { compatibility, hasItems, isSearch } = this;
  ...
  [listItemTemplate](item, index) {
    const { compatibility, twoLines } = this;
  ...
  [cookieEditorTemplate]() {
    const { editorOpened, compatibility } = this;
