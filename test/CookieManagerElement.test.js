...
import { resetSearch, beforeQueryItemsValue, deleteCookies, deleteHandler, getCookieIndex, nextInsertAtPositionValue, exportAllHandler, exportItemsValue } from '../src/CookieManagerElement.js';

/** @typedef {import('@advanced-rest-client/arc-types').Cookies.ARCCookie} ARCCookie */
/** @typedef {import('../src/CookieManagerElement').CookieManagerElement} CookieManagerElement */
...
describe('CookieManagerElement', () => {
  ...
  describe('#listHidden', () => {
    ...
    it('returns false when has items', () => {
      element.items = generator.generateCookiesData({
        size: 1
      });
      assert.isFalse(element.listHidden);
    });
  ...
  describe('[resetSearch]()', () => {
    ...
    it('sets "items" to "[beforeQueryItemsValue]"', () => {
      element[beforeQueryItemsValue] = generator.generateCookiesData({
        size: 50
      });
      element[resetSearch]();
  ...
  describe('[deleteCookies]()', () => {
    ...
    beforeEach(async () => {
      element = await basicFixture();
      cookies = generator.generateCookiesData({
        size: 1
      });
      await aTimeout(0);
    });
  ...
  describe('[getCookieIndex]()', () => {
    ...
    let element = /** @type CookieManagerElement */ (null);
    beforeEach(async () => {
      const items = generator.generateCookiesData({
        size: 3
      });
